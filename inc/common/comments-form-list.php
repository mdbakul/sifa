...
function custom_comment_form_fields($fields) {
    ...
    $fields = array(
        'author' => '<div class="row"><div class="col-xl-6 col-lg-6 col-12 mb-20"><div class="tp-contact-form-input-box">' .
            '<input type="text" name="author" id="author" placeholder="' . esc_attr__('Name*', 'sifa') . '" value="' . esc_attr($commenter['comment_author']) . '" ' . ($req ? 'required' : '') . '></div></div>',

        'email' => '<div class="col-xl-6 col-lg-6 col-12 mb-20"><div class="tp-contact-form-input-box">' .
            '<input type="email" name="email" id="email" placeholder="' . esc_attr__('Email', 'sifa') . '" value="' . esc_attr($commenter['comment_author_email']) . '" ' . ($req ? 'required' : '') . '></div></div>',
        
        'url' => '<div class="col-xl-12 col-lg-12 col-12 mb-20"><div class="tp-contact-form-input-box">' .
            '<input type="text" name="url" id="url" placeholder="' . esc_attr__('Website', 'sifa') . '" value="' . esc_attr($commenter['comment_author_url']) . '"></div></div></div>',
    );
...
function custom_comment_form_comment($comment_field) {
    $comment_field = '<div class="row"><div class="col-12 mb-20"><div class="tp-contact-form-input-box">' .
        '<textarea id="comment" name="comment" placeholder="' . esc_attr__('Your Comment Here...', 'sifa') . '" required></textarea></div></div> </div>';
...
function custom_comment_form_agree($fields) {
    $fields['cookies'] = '<div class="col-xxl-12"><div class="postbox__comment-agree d-flex align-items-start mb-25">' .
        ...
        '<label class="e-check-label" for="e-agree">' . esc_html__('Save my name, email, and website in this browser for the next time I comment.', 'sifa') . '</label></div></div>';
...
function custom_comment_form_submit_button($submit_button) {
    $submit_button = '<div class="sifa-comments-btn">' .
        '<button type="submit" class="tp-btn">' . esc_html__('Submit Comment', 'sifa') . '</button></div>';
...
function custom_comment_list($comment, $args, $depth) {
    ...
    if ($comment->comment_type == 'pingback' || $comment->comment_type == 'trackback') {
        ...
        ?>
        <li class="pingback">
            <p><?php esc_html_e('Pingback:', 'sifa'); ?> <?php comment_author_link(); ?></p>
        </li>
        <?php
    } else {
        ...
        <li <?php comment_class('comment'); ?> id="comment-<?php comment_ID(); ?>"> 
            ...
            <div class="commentsarea">
                ...
                <?php if ($comment->comment_approved == '0') : ?>
                    <p><?php esc_html_e('Your comment is awaiting moderation.', 'sifa'); ?></p>
                <?php endif; ?>
                <?php comment_text(); ?>
            </div>
        <?php
