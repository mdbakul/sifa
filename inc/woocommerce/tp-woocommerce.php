...
if( !function_exists('sifa_content_single_details') ) {
    function sifa_content_single_details( ) {
        global $product;
        global $post;
        global $woocommerce;
    ...
    <div class="tp-shop-details__right-warp">
        ...
        <?php if(!empty($rating)) : ?>
            <div class="tp-shop-details__ratting d-flex">
                ...
                <span class="review-text">( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'sifa'); echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
            </div>
            <?php else : ?>
            <div class="tp-shop-details__ratting">
                ...
                <span><i class="fal fa-star"></i></span>
                <span><i class="fal fa-star"></i></span>
                <span>( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'sifa'); echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
            </div>
        <?php endif; ?> 
...
add_action( 'woocommerce_single_product_summary', 'sifa_content_single_details', 4 );
...
*************************************************/
function sifa_shipping_progress_bar() {
...
if(get_theme_mod( 'shipping_progress_bar_location_card_page',0) == '1'){
    add_action( 'woocommerce_before_cart_table',  'sifa_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_mini_cart',0) == '1'){
    add_action( 'woocommerce_before_mini_cart_contents', 'sifa_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_checkout',0) == '1'){
    add_action( 'woocommerce_checkout_before_customer_details', 'sifa_shipping_progress_bar' );
}
...
if ( ! function_exists( 'sifa_header_add_to_cart_fragment' ) ) {
    function sifa_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        ?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'sifa_header_add_to_cart_fragment' );


// product-content archive
if( !function_exists('sifa_content_product_grid') ) {
    function sifa_content_product_grid( ) {
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
        ...
        <div class="product__thumb w-img p-relative fix">
            ...
            <div class="product__action d-flex flex-column flex-wrap">
                ...
                <div class="product-action-btn product-add-wishlist-btn p-relative">
                    ...
                    <span class="product-action-tooltip"><?php echo esc_html__('Add To Wishlist','sifa'); ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="product-action-btn p-relative">
                    ...
                    <span class="product-action-tooltip"><?php echo esc_html__('Quick view','sifa'); ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="product-action-btn p-relative">
                    ...
                    <span class="product-action-tooltip"> <?php echo esc_html__('Add To Compare','sifa'); ?></span>
                </div>
                <?php endif; ?>
...
add_action( 'woocommerce_before_shop_loop_item', 'sifa_content_product_grid', 10 );


// smart quickview
add_filter( 'woosq_button_html', 'sifa_woosq_button_html', 10, 2 );
function sifa_woosq_button_html( $output , $prodid ) {
    ...
    $price_class = $price ? NULL : 'price-empty';
        ...
        if ( $product ) {
         ...
         if( $product->is_type( 'simple' ) ){
            $btntext = esc_html__("Add to Cart",'sifa');
         } elseif( $product->is_type( 'variable' ) ){
            $btntext = esc_html__("Select Options",'sifa');
         } elseif( $product->is_type( 'external' ) ){
            $btntext = esc_html__("Buy Now",'sifa');
         } elseif( $product->is_type( 'grouped' ) ){
            $btntext = esc_html__("View Products",'sifa');
         }
         else{
            $btntext = "Add to Cart";
         } 
...
/*************************************************
## sifa Woo Search Form
*************************************************/ 

add_filter( 'get_product_search_form' , 'sifa_custom_product_searchform' );

function sifa_custom_product_searchform( $form ) {

	$form = '<div class="search-form p-relative">
                <form action="' . esc_url( home_url( '/shop'  ) ) . '" role="search" method="get" id="searchform">
                <input type="search" value="' . get_search_query() . '" name="s" placeholder="'.esc_attr__('Search Product','sifa').'" autocomplete="off">
